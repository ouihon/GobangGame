...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONDOWN:
        if (!bIsBlackTurn) {
            MessageBox(hwnd, TEXT("当前是白棋回合，请使用键盘进行操作！"), TEXT("提示"), MB_OK);
            return 0;
        }
        ...
        // 获取棋子
        int currentChessFlag = BLACK_FLAG;
        // 记录当前步
        ...
        if (TRUE == IsSomeoneWin(logicalPos, currentChessFlag)) {
            if (IDYES == MessageBox(hwnd, TEXT("黑方获胜！是否重新开始？"), TEXT("提示"), MB_YESNO)) {
                ClearChessBoard();
                InvalidateRect(hwnd, NULL, TRUE);
            } else {
                DestroyWindow(hwnd);
    ...
    case WM_KEYDOWN:
        if (bIsBlackTurn) {
            MessageBox(hwnd, TEXT("当前是黑棋回合，请使用鼠标进行操作！"), TEXT("提示"), MB_OK);
            return 0;
        }
        ...
        switch (wParam) {
            case VK_LEFT:
                ...
                break;
            case VK_RIGHT:
                if (cursorPos.x < BOARD_CELL_NUM) cursorPos.x++;
                break;
            case VK_UP:
                if (cursorPos.y > 0) cursorPos.y--;
                break;
            case VK_DOWN:
                if (cursorPos.y < BOARD_CELL_NUM) cursorPos.y++;
                break;
            case VK_SPACE:
            case VK_RETURN:
                hdc = GetDC(hwnd);
                ...
                // 获取棋子
                int currentChessFlag = WHITE_FLAG;
                // 记录当前步
                ...
                if (TRUE == IsSomeoneWin(cursorPos, currentChessFlag)) {
                    if (IDYES == MessageBox(hwnd, TEXT("白方获胜！是否重新开始？"), TEXT("提示"), MB_YESNO)) {
                        ClearChessBoard();
                        InvalidateRect(hwnd, NULL, TRUE);
                    } else {
                        DestroyWindow(hwnd);
